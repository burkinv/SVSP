...
#define CONNECT_TO_AAVD3D9_THROUGH_STATIC_WRAPPER

/**
__SVSPGraphics_D3D9::Release_Callback
*/
ULONG   FASTCALL __SVSPGraphics_D3D9::Release_Callback              (__SVSPGraphics_D3D9 *pkThis, LPD3DXDEVICE pkDevice)
{
  ...
  return pkDevice->Release();
}

/**
__SVSPGraphics_D3D9::Reset_Callback
*/
HRESULT FASTCALL __SVSPGraphics_D3D9::Reset_Callback                (__SVSPGraphics_D3D9 *pkThis, LPD3DXDEVICE pkDevice, D3DPRESENT_PARAMETERS* pPresentationParameters)
{
...
/**
__SVSPGraphics_D3D9::Present_Callback
*/
HRESULT FASTCALL __SVSPGraphics_D3D9::Present_Callback              (__SVSPGraphics_D3D9 *pkThis, LPD3DXDEVICE pkDevice, CONST RECT* pSourceRect,CONST RECT* pDestRect,HWND hDestWindowOverride,CONST RGNDATA* pDirtyRegion)
{
  pkThis->m_bRender = !pkThis->m_bReleased;
  return pkDevice->Present(pSourceRect,pDestRect,hDestWindowOverride,pDirtyRegion);
}

/**
__SVSPGraphics_D3D9::EndScene_Callback
*/
HRESULT FASTCALL __SVSPGraphics_D3D9::EndScene_Callback             (__SVSPGraphics_D3D9 *pkThis, LPD3DXDEVICE pkDevice, D3DPRESENT_PARAMETERS* pPresentationParameters)
{
  if(pkThis->m_bRender)
  {
    if(pkThis->m_pkHDD != NULL)
    {
      pkThis->SetShaders();
      pkThis->m_pkHDD->Render(pkDevice);
      pkThis->ResetShaders();
    }
...
/**
__SVSPGraphics_D3D9::CreateTexture_Callback
*/
HRESULT FASTCALL __SVSPGraphics_D3D9::CreateTexture_Callback        (__SVSPGraphics_D3D9 *pkThis, LPD3DXDEVICE pkDevice, UINT Width,UINT Height,UINT Levels,DWORD Usage,D3DFORMAT Format,D3DPOOL Pool,IDirect3DTexture9** ppTexture,HANDLE* pSharedHandle)
{
  HRESULT hResult = S_OK;

  if(pkThis->m_bReleased)
  {
    pkThis->Reset(pkDevice, NULL);
  }

  LPD3DXTEXTURE pkTexture = NULL;

  //Set HUD texture only when HUD module is present.
  if((Width == 4) && (Height == 512))
  {
    if(pkThis->m_pkHUD != NULL)
    {
      *pkTexture = pkThis->m_pkHUD->CreateObjectTexture(Width, Height, Levels, Usage, Format, Pool, pSharedHandle);
    }
  }
  else 
  //Set HDD texture only when HDD module is present.
  if((Width == 1024) && (Height == 1024) && (Format == D3DFMT_A1R5G5B5))
  {
    if(pkThis->m_pkHDD != NULL)
    {
      //HDD Texture might be resized.
      *pkTexture = pkThis->m_pkHDD->CreateObjectTexture(Width, Height, Levels, Usage, Format, Pool, pSharedHandle);
    }
  }
...
/**
__SVSPGraphics_D3D9::SetTexture_Callback
*/
HRESULT FASTCALL __SVSPGraphics_D3D9::SetTexture_Callback          (__SVSPGraphics_D3D9 *pkThis, LPD3DXDEVICE pkDevice, DWORD Stage,IDirect3DBaseTexture9* pTexture)
{
  if( (pkThis->m_pkHUD != NULL) && (pkThis->m_pkHUDTexture != NULL) && (pTexture == pkThis->m_pkHUDTexture) )
  {
    //Register__SVSPD3D9_CALLBACK_DRAW_INDEXED_PRIMITIVE for the upcomming HUD stencil rendering (Only in HUD::DISPLAY_MODE_HUD).
    __SVSPD3D9_RegisterDrawIndexedPrimitiveCallback((__SVSPD3DDevice9_DrawIndexedPrimitive_Callback)__SVSPGraphics_D3D9::DrawIndexedPrimitive_Callback, pkThis);
  }
  else 
  if( (pkThis->m_pkHDD != NULL) && (pkThis->m_pkHDDTexture != NULL) &&(pTexture == pkThis->m_pkHDDTexture) )
  {
    //Replace the default 16Bit gauge texture with 32Bit HDD display texture.
    pTexture = pkThis->m_pkHDD->GetDisplayTexture();
  }
  return pkDevice->SetTexture(pkDevice, Stage,pTexture);
}

/**
__SVSPGraphics_D3D9::DrawIndexedPrimitive_Callback
  Must be called ones per frame whenever HUD stencil is set as an active texture, then unregistered.
*/
HRESULT FASTCALL __SVSPGraphics_D3D9::DrawIndexedPrimitive_Callback (__SVSPGraphics_D3D9 *pkThis, LPD3DXDEVICE pkDevice, D3DPRIMITIVETYPE PrimitiveType,INT BaseVertexIndex,UINT MinVertexIndex,UINT NumVertices,UINT startIndex,UINT primCount)
{
  HRESULT                hResult  = S_OK;
  ...
  if((!pkThis->m_bReleased) && (pTexture == pkThis->m_pkHUDTexture))
  {
    ...
    //Render HUD/HMD
    pkThis->SetShaders();
    pkThis->m_pkHUD->Render(pkDevice);
    pkThis->ResetShaders();
...
__SVSPGraphics_D3D9::__SVSPGraphics_D3D9() : __SVSPGraphics(),
  m_bReleased                    (TRUE),
  ...
  m_dwDepthStencilTestEnable     (0)
{
...
#if defined(CONNECT_TO_AAVD3D9_THROUGH_STATIC_WRAPPER)
  //Connect to __SVSPD3D9 layer through the static member wrapers.
  __SVSPD3D9_RegisterCallback(__SVSPD3D9_CALLBACK_RELEASE,                (__SVSPD3D9_Callback)&__SVSPGraphics_D3D9::Release_Callback,              this);
  __SVSPD3D9_RegisterCallback(__SVSPD3D9_CALLBACK_RESET,                  (__SVSPD3D9_Callback)&__SVSPGraphics_D3D9::Reset_Callback,                this);
  //__SVSPD3D9_RegisterCallback(__SVSPD3D9_CALLBACK_PRESENT,                (__SVSPD3D9_Callback)&__SVSPGraphics_D3D9::Present_Callback,              this);
  //__SVSPD3D9_RegisterCallback(__SVSPD3D9_CALLBACK_END_SCENE,              (__SVSPD3D9_Callback)&__SVSPGraphics_D3D9::EndScene_Callback,             this);
  __SVSPD3D9_RegisterCallback(__SVSPD3D9_CALLBACK_CREATE_TEXTURE,         (__SVSPD3D9_Callback)&__SVSPGraphics_D3D9::CreateTexture_Callback,        this);
  __SVSPD3D9_RegisterCallback(__SVSPD3D9_CALLBACK_SET_TEXTURE,            (__SVSPD3D9_Callback)&__SVSPGraphics_D3D9::SetTexture_Callback,           this);
#else
  //Connect to __SVSPD3D9 layer through non-static members directly.
  VOID (FASTCALL __SVSPGraphics_D3D9::*__ReleaseMethodCallback)() = &__SVSPGraphics_D3D9::Release;
  __SVSPD3D9_RegisterCallback(__SVSPD3D9_CALLBACK_RELEASE,                *(__SVSPD3D9_Callback*)&__ReleaseMethodCallback,                         this);

  VOID (FASTCALL __SVSPGraphics_D3D9::*__ResetMethodCallback)(LPD3DXDEVICE, DWORD *)  = &__SVSPGraphics_D3D9::__Reset;
  __SVSPD3D9_RegisterCallback(__SVSPD3D9_CALLBACK_RESET,                  *(__SVSPD3D9_Callback*)&__ResetMethodCallback,                           this);
  ...
  //m_pkFont = new D3DXFONTCUSTOM(this, "Courier New", 20, __SVSPFont_D3D9::BOLD);
  //m_pkFont = new D3DXFONTCUSTOM(this, "Makisupa", 20, __SVSPFont_D3D9::BOLD);
  //m_pkFont = new D3DXFONTCUSTOM(this, "Novamono", 30, __SVSPFont_D3D9::BOLD);
  //m_pkFont = new D3DXFONTCUSTOM(this, "NORMT___", 18, __SVSPFont_D3D9::BOLD);

  m_pkFont = new D3DXFONTCUSTOM(this, "ProFontWindows", 16, __SVSPFont_D3D9::BOLD);

  //Clear state blocks.
  memset(&m_pkStateBlock[0], 0, sizeof(m_pkStateBlock));
}

__SVSPGraphics_D3D9::~__SVSPGraphics_D3D9()
{
...
// Called from __SVSPD3D9 layer as a __SVSPD3DDevice9_Release_Callback.
VOID __SVSPGraphics_D3D9::Release()
{
  ...
  /////////////////////////////////////////////////////////////////////
  //Release __SVSPAvionics modules.
  /////////////////////////////////////////////////////////////////////
  m_pkAvionics->Release();

  m_bReleased = true;
  __SVSPDEBUG::OUTPUT("__SVSPGraphics_D3D9: RELEASED");
  return;
}
...
// Called from __SVSPD3D9 layer as a __SVSPD3DDevice9_Reset_Callback.
VOID __SVSPGraphics_D3D9::Reset()
{
  m_pkDevice = pkDevice;
  ...
  if(m_pkHUD == NULL)
  {
    m_pkHUD = static_cast<HUD *>(m_pkAvionics->GetModule(__SVSPAvionics::AVIONICS_MODULE_HUD));
  }

  if(m_pkHDD == NULL)
  {
    m_pkHDD = static_cast<HDD *>(m_pkAvionics->GetModule(__SVSPAvionics::AVIONICS_MODULE_HDD));
  }
  ...
  for(UINT i = 1; i < 8; ++i)
  {
    m_pkDevice->SetTextureStageState( i, D3DTSS_COLOROP,   D3DTOP_DISABLE );
    m_pkDevice->SetTextureStageState( i, D3DTSS_ALPHAOP,   D3DTOP_DISABLE );
  }
  ...
  /////////////////////////////////////////////////////////////////////
  //Reset __SVSPAvionics modules.
  /////////////////////////////////////////////////////////////////////
  m_pkAvionics->Reset(pkDevice, pParameters);

  m_bReleased = false;
  __SVSPDEBUG::OUTPUT("__SVSPGraphics_D3D9: RESET");
  return;
}

HRESULT  __SVSPGraphics_D3D9::SetHUDStencilSurface()
{
  HRESULT hResult = S_OK;
...
HRESULT  __SVSPGraphics_D3D9::DisableHUDStencilSurface()
{
  HRESULT hResult = S_OK;
...
VOID __SVSPGraphics_D3D9::SetGlobalMaterial(D3DMATERIAL9 *pkMaterial)
{
  m_pkDevice->SetMaterial(pkMaterial);
}


VOID __SVSPGraphics_D3D9::SetShaders() 
{
  //m_pkDevice->SetRenderState( D3DRS_ALPHABLENDENABLE,      TRUE );
  //m_pkDevice->SetRenderState( D3DRS_SRCBLEND,              D3DBLEND_SRCALPHA );
  //m_pkDevice->SetRenderState( D3DRS_DESTBLEND,             D3DBLEND_INVSRCALPHA );
...
VOID __SVSPGraphics_D3D9::ResetShaders() 
{
}

VOID __SVSPGraphics_D3D9::SetScissorRect(const RECT* pRect) 
{
  if(pRect != NULL)
  {
...
VOID  __SVSPGraphics_D3D9::GetViewMatrix(D3DXMATRIX *pkViewMatrix)
{
  m_pkDevice->GetTransform(D3DTS_VIEW, pkViewMatrix);
}

VOID  __SVSPGraphics_D3D9::SetViewMatrix(D3DXMATRIX *pkViewMatrix)
{
  m_pkDevice->SetTransform(pkDevice, D3DTS_VIEW, pkViewMatrix);
}

VOID  __SVSPGraphics_D3D9::SetViewMatrix(D3DXVECTOR3 *pkEyeVector, D3DXVECTOR3 *pkLookAtVector, D3DXVECTOR3 *pkUpVector)
{
  D3DXMATRIXA16 kViewMatrix;
  D3DXMatrixLookAtLH( &kViewMatrix, pkEyeVector, pkLookAtVector, pkUpVector );
  m_pkDevice->SetTransform(pkDevice, D3DTS_VIEW, &kViewMatrix);
}

VOID __SVSPGraphics_D3D9::SetTransform(D3DXMATRIX *pkTransformMatrix) 
{
  AAV_ASSERT_RETURN_VOID(pkTransformMatrix != NULL);
  m_pkDevice->SetTransform(pkDevice, D3DTS_WORLD, pkTransformMatrix);
}
